...
// LLAMA_CLOUD_ADMIN_BASE_URL). This should be used to initialize new clients.
func DefaultClientOptions() []option.RequestOption {
	defaults := []option.RequestOption{option.WithHTTPClient(defaultHTTPClient()), option.WithEnvironmentProduction()}
	if o, ok := os.LookupEnv("LLAMA_CLOUD_ADMIN_BASE_URL"); ok {
		defaults = append(defaults, option.WithBaseURL(o))
	}
	if o, ok := os.LookupEnv("LLAMA_CLOUD_API_KEY"); ok {
		defaults = append(defaults, option.WithAPIKey(o))
	}
	if o, ok := os.LookupEnv("LLAMA_CLOUD_ADMIN_CUSTOM_HEADERS"); ok {
...
// environment (LLAMA_CLOUD_API_KEY, LLAMA_CLOUD_ADMIN_BASE_URL). The option passed
